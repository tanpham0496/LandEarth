...
async function getBitaminHistory({ userId, category, offsetNumber }) {
    try {
        const offset = offsetNumber === 1 ? 0 : offsetNumber * 50;
        const bitaminHistories = await db.BitaminHistory.find({ userId, category }).sort({"updatedAt" : -1}).skip(offset).limit(50).lean();
        // console.log('bitaminHistories', bitaminHistories);
