...
async function getOnLandObjectsByUserId({userId}) {
    try {
        const onLandObjects = await db.Object.find({
            deletedTree: false,
            userId: ObjectId(userId)
        }).populate('item').lean();
...
async function getDetailObject({userId, objectId}) {
    try {
        var treeDetail = await db.Object.findOne({
            userId: ObjectId(userId),
...
async function getObjectByCategories({userId, cateIds, quadKeys, action}) {
    ...
    try {
        if (action === "plant") {
            ...
            const lands = await db.Land23.find({
                "user._id": ObjectId(userId),
                $or: [{categoryId: {$in: cateIds}}, {quadKey: {$in: quadKeys}}]
            }).select("quadKey sellPrice name forSaleStatus categoryId");
            const allQuadKeys = lands.map(land => land.quadKey);
            const trees = await db.Object.find({
                userId: ObjectId(userId),
                $or: [{quadKey: {$in: allQuadKeys}}, {bigTreeQuadKeys: {$elemMatch: {$in: allQuadKeys}}}]
            });
            //remove exist normal tree and bitamin tree
            const landWithoutTree = lands.filter(land => !land.forSaleStatus && !trees.some(tree => tree.quadKey === land.quadKey || (tree.bigTreeQuadKeys && tree.bigTreeQuadKeys.includes(land.quadKey))));
            const landWithoutForSale = lands.filter(land => !land.forSaleStatus);
            if (_.isEmpty(landWithoutForSale)) {
                return {status: false, err: 'landIsForSale'};
            }
            if (_.isEmpty(landWithoutTree)) {
                return {status: false, err: 'noTreeForPlant'};
            }
            ...
            const landWithTreeExceptBitamin = lands.reduce((total, land) => {
                const fTree = trees.find(tree => tree && tree.quadKey === land.quadKey && tree.deletedTree === false && !land.forSaleStatus) ;
                if (fTree) total = total.concat(fTree);
                return total;
            }, []);
            const landWithoutForSale = lands.filter(land => !land.forSaleStatus);
            if (_.isEmpty(landWithoutForSale)) return {status: false, err: 'landIsForSale'};
            ...
            if (_.isEmpty(landWithoutNutrient)) {
                const hasTreeExists = await db.Object.findOne({
                    userId: ObjectId(userId),
                    quadKey: {$in: allQuadKeys},
                    waterEndTime: {$gte: new Date()},
                    bigTreeQuadKeys: null
