...
async function createUsers(numberUSer, landConfig, quadKeyList22){
    const origin = 262144;
    const range = 16;
    ...
    try {
        for(let i = 1; i <= limitNumberUSer; i++){
            ...
            if((i+1)*range <= origin){
                //buy lands
                const arrLand22 = quadKeyList22.slice(i*range, (i+1)*range);
...
const { quadKeyList22, vitualDatas, vitualDatas2, splitCategory, list } = require('./quadKeyList');
...
(async () => {
    ...
    if(process.env.NODE_ENV === 'development'){
        ...
        console.log('-----------------------------------------');
        await createUsers(262144, landConfig, quadKeyList22);
        console.log('-----------------------------------------');
